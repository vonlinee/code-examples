...
public abstract class EventServiceAction extends AbstractAction {
     ...
     * Action#ACTION_COMMAND_KEY} <li>the action event's {@link Action#ACTION_COMMAND_KEY}
     * <li>the action's {@link Action#NAME} the value is used (if the value is not a String, the value's
     * toString() is used).
     * <p/>
     ...
     * <p/>
     * Gets the EventService from {@link #getEventService(ActionEvent)}. Checks isPublishesOnTopic().  If true,
     * gets the topic name from {@link #getTopicName(ActionEvent)} and the topic value from {@link
     * #getTopicValue(ActionEvent)}, and publishes the value on the topic on the EventService.  If false, gets event from
     * {@link #getEventServiceEvent(ActionEvent)}, and publishes the event on the EventService.
     * <p/>
     *
