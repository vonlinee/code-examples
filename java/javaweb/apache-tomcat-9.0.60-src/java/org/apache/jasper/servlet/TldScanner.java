...
public class TldScanner {
    ...
    public void scanJars() {
    	...
    	LogUtils.log("开始扫描webapps[%s]下的/WEB-INF/lib目录下的jar包中的tld文件", context.getContextPath());
        ...
        if (callback.scanFoundNoTLDs()) {
            ...
            log.info("At least one JAR was scanned for TLDs yet contained no TLDs. Enable debug logging for this logger for a complete list of JARs that were scanned but no TLDs were found in them. Skipping unneeded JARs during scanning can improve startup time and JSP compilation time.");
        }
