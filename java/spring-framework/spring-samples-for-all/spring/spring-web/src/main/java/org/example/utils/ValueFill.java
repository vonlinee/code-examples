...
public class ValueFill {
    ...
    private static final ConcurrentHashMap<Class<?>, Object> defaultValueMap = new ConcurrentHashMap<>();
    ...
    public static <T> T createDefaultValueForClass(Class<?> classType) {
        ...
        if (classType.isPrimitive()) {
            ...
            if (List.class.isAssignableFrom(classType)) {
                emptyObject = new ArrayList<>();
            } else if (Map.class.isAssignableFrom(classType)) {
                emptyObject = new HashMap<>();
            } else if (Set.class.isAssignableFrom(classType)) {
                emptyObject = new HashSet<>();
            }
            return (T) emptyObject; // 其余的接口暂不考虑
        ...
        if (defaultValueMap.containsKey(classType)) {
            ...
            // 普通对象类型
            emptyObject = newInstance(classType); // 创建空对象
    ...
    public static <T> T createDefaultValueForGenericCustomClass(Class<?> rawClass, Type[] actualTypeArguments) {
        ...
        Map<String, Type> typeVariableMapping = new HashMap<>();
    ...
    public static <T> T createDefaultValueForParameterizedType(ParameterizedType parameterizedType) {
        ...
        if (rawType instanceof Class<?>) {
            ...
            if (List.class.isAssignableFrom(rawClass)) {
                ...
                list.add(createDefaultValueForType(actualTypeArgument));
                return (T) list;
            } else if (Map.class.isAssignableFrom(rawClass)) {
                Map<Object, Object> map = new HashMap<>();
                map.put(createDefaultValueForType(actualTypeArguments[0]), createDefaultValueForType(actualTypeArguments[1]));
                return (T) map;
            } else if (Set.class.isAssignableFrom(rawClass)) {
                Set<Object> set = new HashSet<>();
                set.add(createDefaultValueForType(actualTypeArguments[0]));
                return (T) set;
            } else {
                // TODO 其他泛型对象
