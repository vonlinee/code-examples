...
import io.spring.boot.dao.IUserDao;
import io.spring.boot.common.entity.Role;
...
public class MyShiroRealm extends AuthorizingRealm {
     ...
     * 经测试：如果连续访问同一个URL（比如刷新），该方法不会被重复调用，Shiro有一个时间间隔（使用的ehcache，时间在ehcache-shiro.xml中配置），超过这个时间间隔再刷新页面，该方法会被执行
     */
    @Override
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        if (user != null) {
            ...
            List<
                    Role> roleList = user.getRoleList();
