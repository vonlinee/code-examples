...
public class ShardingDataSourceConfig {
	...
	public DataSource dataSource() throws SQLException {
		...
		DataSource dataSource = ShardingDataSourceFactory.createDataSource(dataSourceMap, shardingRuleConfig, configMap,
				propertie);
		...
		// OrchestrationConfiguration("orchestration-sharding-data-source", regConfig,
		// true);
		// 5.3、获取数据源对象
		// DataSource dataSource =
		// OrchestrationShardingDataSourceFactory.createDataSource(dataSourceMap,
		// shardingRuleConfig, configMap, propertie, orchConfig);
		return dataSource;
	}

	@Bean(name = "shardingSqlSessionFactory")
	public SqlSessionFactory sqlSessionFactorySharding(@Qualifier("shardingDataSource") DataSource dataSource)
			throws Exception {
		...
		bean.setMapperLocations(new PathMatchingResourcePatternResolver()
				.getResources("classpath:mybatis/mapper/shardingMapper/**/*.xml"));
	...
	// transactionManagerXA(@Qualifier("shardingDataSource") DataSource dataSource)
	// {
	// return new AtomikosTransactionManager();
	// }

	@Bean(name = "shardingSqlSessionTemplate")
	public SqlSessionTemplate sqlSessionTemplateThree(
			@Qualifier("shardingSqlSessionFactory") SqlSessionFactory sqlSessionFactory) throws Exception {
		return new SqlSessionTemplate(sqlSessionFactory);
	}

	/**
	 * Sharding提供了5种分片策略： StandardShardingStrategyConfiguration:标准分片策略, 提供对SQL语句中的=,
	 * IN和BETWEEN AND的分片操作支持 ComplexShardingStrategyConfiguration:复合分片策略,
	 * 提供对SQL语句中的=, IN和BETWEEN AND的分片操作支持。
	 * InlineShardingStrategyConfiguration:Inline表达式分片策略,
	 * 使用Groovy的Inline表达式，提供对SQL语句中的=和IN的分片操作支持
	 * HintShardingStrategyConfiguration:通过Hint而非SQL解析的方式分片的策略
	 * NoneShardingStrategyConfiguration:不分片的策略 Sharding提供了以下4种算法接口：
	 * PreciseShardingAlgorithm RangeShardingAlgorithm HintShardingAlgorithm
	...
	TableRuleConfiguration getOrderTableRuleConfiguration() {
		...
		// HintShardingStrategyConfiguration(new OrderDataBaseHintShardingAlgorithm()));
		...
		// orderPreciseShardingAlgorithm(),new orderRangeShardingAlgorithm()));
		// result.setTableShardingStrategyConfig(new
		// ComplexShardingStrategyConfiguration("order_id,user_id",new
		// orderComplexKeysShardingAlgorithm()));
		// result.setTableShardingStrategyConfig(new
		// HintShardingStrategyConfiguration(new OrderTableHintShardingAlgorithm()));
		// 6、指定自增字段以及key的生成方式
	...
	class orderPreciseShardingAlgorithm implements PreciseShardingAlgorithm<Long> {
		...
		public String doSharding(Collection<String> collection, PreciseShardingValue<Long> preciseShardingValue) {
			logger.info("collection:" + JSON.toJSONString(collection) + ",preciseShardingValue:"
	...
	class orderRangeShardingAlgorithm implements RangeShardingAlgorithm<Long> {
		...
		public Collection<String> doSharding(Collection<String> collection,
			...
			logger.info("Range collection:" + JSON.toJSONString(collection) + ",rangeShardingValue:"
	...
	class orderComplexKeysShardingAlgorithm implements ComplexKeysShardingAlgorithm {
		...
		public Collection<String> doSharding(Collection<String> collection, Collection<ShardingValue> shardingValues) {
			logger.info("collection:" + JSON.toJSONString(collection) + ",shardingValues:"
