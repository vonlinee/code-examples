package org.example.java8.io.nio.chat;
...
public class ChatClient {
    ...
    public void startClient() throws IOException {
        ...
        /*
         * 因为等待用户输入会导致主线程阻塞 所以用主线程处理输入，新开一个线程处理读数据
         */
        ...
        while (flag) {
            ...
            if ("".equals(name)) {  //姓名如果没有初始化
                //啥也不干，之后发给服务端验证姓名
                System.out.println("姓名未初始化");
    ...
    private class ClientReadThread implements Runnable {
        ...
        public void run() {
            ...
            try {
                while (flag) {
                    int count = selector.select();//调用此方法一直阻塞，直到有channel可用
                    if (count <= 0) continue;
