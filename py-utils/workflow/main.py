...
def make_graphql_field(template_path: str, field_type: str, field_name: str, output_dir: str = None) -> None:
    ...
    content = field_type + " " + field_name + \
        " {" + read_excel(template_path) + "\n}"
...
def create_graphqls_content():
    ...
    make_graphql_field(default_template_dir + r"字段描述模板.xlsx",
                       "type", "VeMktActInfoActMode")
...
def lines_to_empty_json(input_file: str = INPUT_TXT_FILE_PATH):
    ...
    json_str = json.dumps(empty_key_json, sort_keys=True,
                          indent=4, separators=(',', ': '))
    print(json_str)
...
def delete_files(file_name: str) -> None:
    if file_name.endswith('.project') or file_name.endswith('.classpath'):
        os.remove(file_name)
        print("删除" + file_name)
...
def delete_directory(dir: str) -> None:
    try:
        if dir.endswith('bin') or dir.endswith('target') or dir.endswith('.settings'):
            os.removedirs(dir)
            print("删除" + dir)
