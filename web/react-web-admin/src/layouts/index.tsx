...
import { Layout, Drawer, MenuProps } from 'antd'
...
import './index.less'
import TabsList = ModelRoute.TabsList

const Layouts = () => {
  // 页面跳转使用useLocation接受传来的值
  const { pathname } = useLocation()
  console.log('pathname = ', pathname)
  // 折叠菜单栏
  const [collapsed, setCollapsed] = useState<boolean>(false)
  // 菜单栏样式
  const [collapsedClass, setCollapsedClass] = useState<string>(useSelector(getNavigationMode) === 1 ? 'site-layouts' : 'site-layouts site-layouts-full')
  // 用户名
  const [userName, setUserName] = useState<string>('X')
    ...
    if (!isSmall) {
      ...
      if (collapsed) {
        ...
        if (navigationMode === 1) {
          ...
          setCollapsedClass(
            'site-layouts site-layouts-full'
          )
        }
      } else {
        ...
        if (navigationMode === 1) {
          setCollapsedClass(
            'site-layouts ant-layouts-collapsed'
          )
        } else {
          setCollapsedClass(
            'site-layouts ant-layouts-collapsed site-layouts-full ant-layouts-collapsed-full'
          )
        }
      }
    } else {
  ...
  const generateTableList = (routeList: any[]) => {
    ...
    routeList.map((ele) => {
      if (ele.path === pathname) {
        ...
        const resultArr = tabsList.filter((item1: TabsList) =>
          pathnameArr.some((item2: string) => item2 === item1.path)
        )
        if (!resultArr.length) {
          // tabsList.push({
          //   path:ele.path,
   ...
   * @param {array} keys
   */
  const openChangeMenu: MenuProps['onOpenChange'] = (
    keys
  ) => {
    // console.log('===0====');
