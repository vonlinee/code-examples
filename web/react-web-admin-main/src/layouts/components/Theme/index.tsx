...
const Theme = (props: ThemeProps) => {
  ...
  const handleChangeSetUp = (
    e: boolean | ModelStoreConfig.Navigation,
    status: string
  ) => {
    switch (status) {
      case 'watermark':
      ...
      case 'navigationMode':
        dispatch(
          setNavigationMode(
            e as ModelStoreConfig.Navigation
          )
        )
        break
    }
      ...
      <div className="theme-item theme-item3">
        ...
        <Tooltip placement="top" title="侧边菜单布局">
          <div
            ...
            onClick={() =>
              handleChangeSetUp(1, 'navigationMode')
            }
          >
            {useSelector(getNavigationMode) === 1 && (
              <img
                src={gouImg}
                className="theme-item-model-img"
              />
            )}
          </div>
        </Tooltip>
        ...
        <Tooltip placement="top" title="顶部菜单布局">
          <div
            ...
            onClick={() =>
              handleChangeSetUp(2, 'navigationMode')
            }
          >
            {useSelector(getNavigationMode) === 2 && (
              <img
                src={gouImg}
                className="theme-item-model-img"
              />
