...
//		// 必须重新初始化 AbstractRoutingDataSource 中的 resolvedDataSources，动态切换才会生效
	...
	private void autowiredDynamicDataSource(DynamicDataSource dynamicDataSource) {
		...
		org.apache.ibatis.mapping.Environment myBatisEnvironment = sqlSessionFactory.getConfiguration()
				.getEnvironment();
		sqlSessionFactory.getConfiguration().setEnvironment(new org.apache.ibatis.mapping.Environment(
				myBatisEnvironment.getId(), myBatisEnvironment.getTransactionFactory(), dynamicDataSource));
	}
	...
	public void addProperties(Environment environment) {
		// 现在的code:
