...
import io.maker.codegen.context.event.ext.EventUtil;

/**
 * Base class for FX events. Each FX event has associated an event source, event
 * target and an event type. The event source specifies for an event handler the
 * object on which that handler has been registered and which sent the event to
 * it. The event target defines the path through which the event will travel
 * when posted. The event type provides additional classification to events of
 * the same {@code Event} class.
 * 
 * @since JavaFX 2.0
 */
public class Event extends EventObject implements Cloneable {
	...
	/**
	 * Event target that defines the path through which the event will travel when
	 * posted.
	 */
	...
	/**
	 * Construct a new {@code Event} with the specified event type. The source and
	 * target of the event is set to {@code NULL_SOURCE_TARGET}.
	 *
	...
	/**
	 * Construct a new {@code Event} with the specified event source, target and
	 * type. If the source or target is set to {@code null}, it is replaced by the
	 * {@code NULL_SOURCE_TARGET} value.
	 *
	...
	/**
	 * Returns the event target of this event. The event target specifies the path
	 * through which the event will travel when posted.
	 *
	...
	/**
	 * Gets the event type of this event. Objects of the same {@code Event} class
	 * can have different event types. These event types further specify what kind
	 * of event occurred.
	 *
	...
	/**
	 * Creates and returns a copy of this event with the specified event source and
	 ...
	 * handler.
	 *
	 * @return {@code true} if this {@code Event} has been consumed, {@code false}
	 *         otherwise
	 */
	 ...
	 * 
	 * @return a new instance of {@code Event} with all values copied from this
	 *         {@code Event}.
	 */
	@Override
	public Object clone() {
	...
	// PENDING_DOC_REVIEW
	/**
	 * Fires the specified event. The given event target specifies the path through
	 * which the event will travel.
	 *
