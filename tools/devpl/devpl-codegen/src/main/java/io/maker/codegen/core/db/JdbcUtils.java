...
import io.maker.codegen.core.db.result.ResultSetColumnMetadata;
import io.maker.codegen.core.db.result.ResultSetHandler;
...
public final class JdbcUtils {
	 ...
	 * 加载连接信息
	 */
	private static final String DEFAULT_PROPERTIES_LOCATION = System.getProperty("user.home") + File.separator
			+ JDBC_PROPERTIES;
	...
	/**
	 * 获取默认连接配置文件 DEFAULT_PROPERTIES_LOCATION System.getProperty("user.home") +
	 * File.separator + JDBC_PROPERTIES ${user.home}/jdbc.properties
	 * 
	 * @return Properties
	 */
	public static Properties getLocalProperties() {
		if (JPROPERTIES_LOCATION.length() > 0) {
	...
	public static boolean isTableExist(final Connection connection, final String catalog, final String table,
		...
		try (ResultSet resultSet = connection.getMetaData().getTables(catalog, getSchema(connection, databaseType),
				table, null)) {
	...
	public static <T> T prepareQuery(Connection conn, String sql, ResultSetHandler<T> handler, Object... args) {
		try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			if (args.length != paramCount) {
				throw new RuntimeException("prepared sql (" + sql + ") needs param size is " + "" + paramCount
						+ ", but provided " + args.length);
			}
	...
	public static Optional<Connection> getConnection(Properties properties) {
		...
		if (result) {
			try {
				...
				LOG.error("failed to get connection".intern(), e);
			}
		} else {
			LOG.error("failed to load driver");
		}
	...
	/**
	 * 获取数据库表信息 在MySQL中，物理上schema和database是等价的。 在MySQL
	 ...
	 * @throws SQLException
	 */
	public static List<Map<String, Object>> resultSetToList(final ResultSet rs) throws SQLException {
