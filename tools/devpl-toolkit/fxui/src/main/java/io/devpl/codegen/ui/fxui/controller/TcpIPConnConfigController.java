...
import io.devpl.codegen.ui.fxui.utils.FXUtils;
...
public class TcpIPConnConfigController extends FXController {
    ...
    void saveConnection() {
        try {
            ...
            if (FXUtils.closeOwnerStage(nameField)) {
                // 加载数据库连接，获取所有的表信息
                FXEvent event = new FXEvent(this, FXEvent.DATA_SEND);
                event.put("db_configs", List.of(assembleDbConnInfoConfiguration()));
                publish(event, target -> target.getClass() == MainController.class);
            }
        } catch (Exception e) {
    ...
    private void initDefaultMySQLConnectionInfo() {
        FXUtils.setTextIfEmpty(hostField, UUID.randomUUID().toString());
