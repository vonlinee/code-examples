...
import io.devpl.codegen.core.plugins.MVCSupportPlugin;
...
public class MyBatisCodeGenerator {
    ...
    public void setGeneratorConfiguration(CodeGenConfiguration generatorConfig) {
    ...
    public void generate() throws Exception {
        ...
        if (generatorConfig.isOffsetLimit()) {
            if (DbType.MYSQL5.name().equals(dbType) || DbType.MYSQL8.name().equals(dbType)
                    || DbType.POSTGRE_SQL.name().equals(dbType)) {
                PluginConfiguration pluginConfiguration = new PluginConfiguration();
        ...
        org.mybatis.generator.api.MyBatisGenerator mbgGenerator = new org.mybatis.generator.api.MyBatisGenerator(configuration, shellCallback, warnings);
        ...
        mbgGenerator.generate(progressCallback, contexts, fullyqualifiedTables);
    }

    private void addPlugins(Context context) {
        PluginConfiguration pc = new PluginConfiguration();
        pc.addProperty("type", MVCSupportPlugin.class.getName());
        pc.setConfigurationType(MVCSupportPlugin.class.getName());
