 ...
 * 1.CodeMirror: CodeMirror编辑器核心对象
 * 2.codemirror: Vue组件对象
 * 3.install: 函数，插件函数，通过Vue.use进行使用
 */
console.log(VueCodeMirror);

let CodeMirror = VueCodeMirror['CodeMirror'];
...
CodeMirror.defineMode('freemarker', function(config) {
  ...
  let settings = {
    leftDelimiter: '<',
    rightDelimiter: '>',
    tagSyntax: 1, // 1 angle_bracket,2 square_bracket
  };
  ...
  let freemarkerStartTagArray = ['#', '@'];

  let freemarkerEndTagArray = ['/#', '/@', '/>'];
  ...
  let parsers = {
    ...
    tokenizer: function(stream, state) {
      if (stream.match(settings.leftDelimiter, true)) {
        if (stream.match('#--', true)) {
          return helpers.chain(stream, state,
    ...
    freemarkerTemplate: function(stream, state) {
      ...
      if (stream.match(settings.leftDelimiter, true)) {
        for (i = 0; i < freemarkerStartTagArray.length; i++) {
          if (stream.match(freemarkerStartTagArray[i], false)) {
            state.depth++;
            if (freemarkerStartTagArray[i] === '@') {
        ...
        for (i = 0; i < freemarkerEndTagArray.length; i++) {
          if (stream.match(freemarkerEndTagArray[i], false)) {
            state.depth++;
            if (freemarkerEndTagArray[i] === '/@') {
      ...
      let ch = stream.next();
      if ('.' === ch) {
        if ('echo' === state.freemarkerMode || 'whitespace' === state.last ||
            'operator' === state.last) {
          for (i = 0; i < specialVariables.length; i++) {
            if (stream.match(specialVariables[i], true)) {
              return helpers.cont('keyword', 'variable',
                  state.freemarkerMode);
            }
          }
        }
        if ('keyword' === state.last &&
            stream.eatWhile(regs.validIdentifier)) {
          return helpers.cont('keyword', null, state.freemarkerMode);
        } else {
          return helpers.cont('operator', 'childVariable',
              state.freemarkerMode);
        }
      ...
      } else if ('tag' === state.freemarkerMode && 'whitespace' ===
          state.last &&
          (stream.match('as', true) || stream.match('in', true) ||
              stream.match('using', true))) {
        return helpers.cont('keyword', 'operator', state.freemarkerMode);
      } else if ('tag' === state.freemarkerMode && 'whitespace' ===
          state.last &&
          (stream.match('gte', true) || stream.match('lte', true) ||
              stream.match('gt', true) || stream.match('lt', true))) {
        return helpers.cont('operator', 'operator', state.freemarkerMode);
      } else {
        if ('builtin' === state.last) {
          ...
          if ('macro' === state.freemarkerMode) {
            stream.eatWhile(regs.validIdentifier);
            return helpers.cont('attribute', 'attribute',
                state.freemarkerMode);
          } else {
            stream.eatWhile(regs.validIdentifier);
          ...
          if ('tag' === state.freemarkerMode) {
            ...
            for (i = 0; i < keyFunctions.length; i++) {
