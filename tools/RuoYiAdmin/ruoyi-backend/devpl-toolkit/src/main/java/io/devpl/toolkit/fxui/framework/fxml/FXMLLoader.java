...
public class FXMLLoader {
    ...
    private abstract class Element {
        ...
        private Object resolvePrefixedValue(String aValue) throws LoadException {
            if (aValue.startsWith(ESCAPE_PREFIX)) {
                ...
                if (aValue.length() == 0 || !(aValue.startsWith(ESCAPE_PREFIX) || aValue.startsWith(RELATIVE_PATH_PREFIX) || aValue.startsWith(RESOURCE_KEY_PREFIX) || aValue.startsWith(EXPRESSION_PREFIX) || aValue.startsWith(BI_DIRECTIONAL_BINDING_PREFIX))) {
        ...
        private Object populateArrayFromString(Class<?> type, String stringValue) throws LoadException {
            ...
            if (stringValue.length() > 0) {
                ...
                for (int i = 0; i < values.length; i++) {
                    Array.set(propertyValue, i, BeanAdapter.coerce(resolvePrefixedValue(values[i].trim()), type.getComponentType()));
                }
            } else {
        ...
        private <T> T getExpressionObjectOfType(String handlerValue, Class<T> type) throws LoadException {
            ...
            if (expression != null) {
                ...
                throw constructLoadException("Error resolving \"" + handlerValue + "\" expression." + "Does not point to a " + type.getName());
            }
            return null;
        }
                    ...
                    if (value instanceof ObservableList && attribute.name.equals(COLLECTION_HANDLER_NAME)) {
                        ...
                        if (eventHandler == null) {
                            if (handlerName.length() == 0 || scriptEngine == null) {
                                throw constructLoadException("Error resolving " + attribute.name + "='" + attribute.value + "', either the event handler is not in the Namespace or there is an error in the script.");
                            }
        ...
        private void processObservableListHandler(String handlerValue) throws LoadException {
            ...
            if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
                ...
                if (listener instanceof ListChangeListener) {
                    ...
                    throw constructLoadException("Error resolving \"" + handlerValue + "\" expression." + "Must be either ListChangeListener or InvalidationListener");
                }
            }
        }
        ...
        private void processObservableMapHandler(String handlerValue) throws LoadException {
            ...
            if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
                ...
                if (listener instanceof MapChangeListener) {
                    ...
                    throw constructLoadException("Error resolving \"" + handlerValue + "\" expression." + "Must be either MapChangeListener or InvalidationListener");
                }
            }
        }
        ...
        private void processObservableSetHandler(String handlerValue) throws LoadException {
            ...
            if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
                ...
                if (listener instanceof SetChangeListener) {
                    ...
                    throw constructLoadException("Error resolving \"" + handlerValue + "\" expression." + "Must be either SetChangeListener or InvalidationListener");
                }
            }
        }
            ...
            if (i != j) {
                ...
                if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
                    ...
                    if (listener instanceof ChangeListener) {
                        ...
                        throw constructLoadException("Error resolving \"" + handlerValue + "\" expression." + "Must be either ChangeListener or InvalidationListener");
                    }
                }
            ...
            if (parent == null) {
                ...
                if (fxNSURI != null) {
                    ...
                    if (compareJFXVersions(FX_NAMESPACE_VERSION, fxVersion) < 0) {
                        throw constructLoadException("Loading FXML document of version " + fxVersion + " by JavaFX runtime supporting version " + FX_NAMESPACE_VERSION);
                    }
                }
            ...
            if (prefix != null && prefix.equals(FX_NAMESPACE_PREFIX)) {
                if (localName.equals(FX_ID_ATTRIBUTE)) {
                    ...
                    if (current.parent != null) {
                        throw constructLoadException(FX_NAMESPACE_PREFIX + ":" + FX_CONTROLLER_ATTRIBUTE + " can only be applied to root element.");
                    }
    ...
    private class IncludeElement extends ValueElement {
        ...
        public Object constructValue() throws IOException {
            ...
            FXMLLoader fxmlLoader = new FXMLLoader(location, resources, builderFactory, controllerFactory, charset, loaders);
    ...
    private class ScriptElement extends Element {
        ...
        public void processStartElement() throws IOException {
            ...
            if (source != null && !staticLoad) {
                ...
                if (engine == null) {
                    throw constructLoadException("Unable to locate scripting engine for" + " extension " + extension + ".");
                }
        ...
        public void processAttribute(String prefix, String localName, String value) throws IOException {
            if (prefix == null && localName.equals(SCRIPT_SOURCE_ATTRIBUTE)) {
                ...
                throw constructLoadException(prefix == null ? localName : prefix + ":" + localName + " is not a valid attribute.");
            }
        }
    }
    ...
    private static class ObservableSetChangeAdapter implements SetChangeListener<Object> {
        public final MethodHandler handler;
    ...
    /**
     * 最终的加载动作
     * @param inputStream FXML输入流
     * @param callerClass 调用者所在类
     * @param <T>
     * @return
     * @throws IOException
     */
    ...
    private <T> T loadImpl(InputStream inputStream, Class<?> callerClass) throws IOException {
        ...
        try {
            ...
            try {
                ...
                XMLStreamReader rawXMLStreamReader = xmlInputFactory.createXMLStreamReader(inputStream, charset.name());

                xmlStreamReader = new StreamReaderDelegate(rawXMLStreamReader) {
                    @Override
    ...
    public static <T> T load(URL location, ResourceBundle resources, BuilderFactory builderFactory) throws IOException {
        return loadImpl(location, resources, builderFactory, (System.getSecurityManager() != null) ? walker.getCallerClass() : null);
    }
    ...
    public static <T> T load(
        ...
        return loadImpl(location, resources, builderFactory, controllerFactory, (System.getSecurityManager() != null) ? walker.getCallerClass() : null);
    }

    private static <T> T loadImpl(
            URL location,
            ResourceBundle resources,
            BuilderFactory builderFactory,
            Callback<Class<?>, Object> controllerFactory,
            Class<?> callerClass) throws IOException {
        return loadImpl(location, resources, builderFactory, controllerFactory, Charset.forName(DEFAULT_CHARSET_NAME), callerClass);
    }
    ...
    public static <T> T load(URL location, ResourceBundle resources, BuilderFactory builderFactory, Callback<Class<?>, Object> controllerFactory, Charset charset) throws IOException {
        return loadImpl(location, resources, builderFactory, controllerFactory, charset, (System.getSecurityManager() != null) ? walker.getCallerClass() : null);
    }
    ...
    private static final class ControllerAccessor {
        ...
        private void addAccessibleMethods(final Class<?> type, final int allowedMemberAccess) {
            ...
            for (int i = 0; i < methods.length; ++i) {
                ...
                // a) it is the method initialize() , or
