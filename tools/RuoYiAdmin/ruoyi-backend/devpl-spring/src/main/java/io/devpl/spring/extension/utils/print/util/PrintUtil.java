    ...
    public static List<String> printRows(List<List<TableCell>> rows, int[] columnWidths, String verticalSep) {
        ...
        for (List<TableCell> row : rows) {
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                switch (alignment) {
                    case LEFT:
                        sb.append(String.format("%s %s %s", verticalSep, StringPadUtil.rightPad(tableCell.getValue(), columnWidths[i]), verStrTemp));
                        break;
                    case RIGHT:
                        sb.append(String.format("%s %s %s", verticalSep, StringPadUtil.leftPad(tableCell.getValue(), columnWidths[i]), verStrTemp));
                        break;
                    case CENTER:
                        sb.append(String.format("%s %s %s", verticalSep, StringPadUtil.center(tableCell.getValue(), columnWidths[i]), verStrTemp));
                        break;
                    default:
