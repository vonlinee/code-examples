package io.devpl.sdk.beans.serialize.json;

import static io.devpl.sdk.beans.serialize.json.JodaBeanJsonWriter.BEAN;
import static io.devpl.sdk.beans.serialize.json.JodaBeanJsonWriter.META;
import static io.devpl.sdk.beans.serialize.json.JodaBeanJsonWriter.TYPE;
import static io.devpl.sdk.beans.serialize.json.JodaBeanJsonWriter.VALUE;
...
import io.devpl.sdk.beans.serialize.SerDeserializer;
import io.devpl.sdk.beans.serialize.SerIterable;
...
abstract class AbstractJsonReader {
     ...
     */
    AbstractJsonReader(final JodaBeanSerializer settings) {
        BeanUtils.notNull(settings, "settings");
    ...
    private Object parseTypedSimple(Class<?> declaredType) throws Exception {
        String typeStr = input.acceptString();
        Class<?> effectiveType = settings.getDeserializers()
                                         .decodeType(typeStr, settings, basePackage, knownTypes, declaredType);
        if (!declaredType.isAssignableFrom(effectiveType)) {
            throw new IllegalArgumentException("Specified type is incompatible with declared type: " +
                    declaredType.getName() + " and " + effectiveType.getName());
        }
