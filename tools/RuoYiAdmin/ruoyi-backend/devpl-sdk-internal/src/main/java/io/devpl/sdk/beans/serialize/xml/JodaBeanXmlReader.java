...
public class JodaBeanXmlReader {
     ...
     */
    public JodaBeanXmlReader(final JodaBeanSerializer settings) {
    ...
    private <T> T read(final Class<T> rootType) throws Exception {
        ...
        if (attr == null && rootType == Bean.class) {
            throw new IllegalArgumentException("Root element attribute must specify '" + JodaBeanXml.TYPE + "'");
        }
    ...
    private Object parseIterable(final StartElement iterableEvent, final SerIterable iterable) throws Exception {
        ...
        while (!event.isEndElement()) {
            if (event.isStartElement()) {
                StartElement start = event.asStartElement();
                QName expectedType = iterable.category() == SerCategory.MAP ? JodaBeanXml.ENTRY_QNAME : JodaBeanXml.ITEM_QNAME;
