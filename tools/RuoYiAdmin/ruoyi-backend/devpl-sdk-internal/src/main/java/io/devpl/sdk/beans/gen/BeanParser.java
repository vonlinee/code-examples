...
class BeanParser {
    ...
    BeanGen parse() {
        ...
        if (parseBeanHierarchy(beanDefIndex).equals("immutable")) {
            ...
            data.setConstructorStyle(BeanGen.CONSTRUCTOR_BY_BUILDER);
        } else if (data.getImmutableConstructor() == BeanGen.CONSTRUCTOR_NONE) {
            if (data.isImmutable()) {
                if (data.isTypeFinal()) {
                    data.setConstructorStyle(BeanGen.CONSTRUCTOR_BY_ARGS);
                } else {
                    data.setConstructorStyle(BeanGen.CONSTRUCTOR_BY_BUILDER);
                }
            } else {
        ...
        if (data.isImmutable()) {
            ...
            if (data.isBeanStyleLight() && !data.isTypeFinal()) {
                throw new BeanCodeGenException("Invalid bean style: Light beans must be declared final", file, beanDefIndex);
            }
            if (data.isBeanStyleMinimal() && !data.isTypeFinal()) {
                throw new BeanCodeGenException("Invalid bean style: Minimal beans must be declared final", file, beanDefIndex);
            }
            if (data.isFactoryRequired() && !data.isRootClass()) {
                throw new BeanCodeGenException("Invalid bean style: Factory method only allowed when bean has no bean superclass", file, beanDefIndex);
            }
            if (data.isFactoryRequired() && !data.isTypeFinal()) {
                throw new BeanCodeGenException("Invalid bean style: Factory method only allowed when bean is final", file, beanDefIndex);
            }
        } else {
            if (data.isBeanStyleLight() && !data.isTypeFinal()) {
                throw new BeanCodeGenException("Invalid bean style: Light beans must be declared final", file, beanDefIndex);
            ...
            if (data.isFactoryRequired()) {
                throw new BeanCodeGenException("Invalid bean style: Factory method only allowed when bean is immutable", file, beanDefIndex);
            }
        }
        ...
        if (data.isImmutable()) {
            for (PropertyGen prop : properties) {
                if (!prop.getData().isDerived() && !prop.getData().isFinal()) {
                    throw new BeanCodeGenException("ImmutableBean must have final properties: " + data.getTypeRaw() + "." + prop
                            .getData().getFieldName(), file, prop.getData().getLineIndex());
                }
            }
        } else {
            if (data.getImmutableConstructor() > BeanGen.CONSTRUCTOR_NONE) {
                throw new BeanCodeGenException("Mutable beans must not specify @ImmutableConstructor: " + data.getTypeRaw(), file, beanDefIndex);
            }
            if (!"smart".equals(data.getConstructorScope()) && !data.isBeanStyleLight()) {
                throw new BeanCodeGenException("Mutable beans must not specify @BeanDefinition(constructorScope): " + data.getTypeRaw(), file, beanDefIndex);
            }
        }
    ...
    private int parseImmutableConstructor(int defLine) {
        int found = BeanGen.CONSTRUCTOR_NONE;
        for (int index = defLine; index < content.size(); index++) {
        ...
        for (int index = defLine; index < content.size(); index++) {
            if (content.get(index).trim().equals("@ImmutableValidator")) {
                ...
                if (index + 1 < content.size()) {
                    ...
                    throw new BeanCodeGenException("@ImmutableValidator method must be private void and no-args", file, index + 1);
                }
            }
        }
        return null;
        ...
        for (int index = defLine; index < content.size(); index++) {
            if (content.get(index).trim().equals("@ImmutableDefaults")) {
                ...
                if (index + 1 < content.size()) {
                    ...
                    throw new BeanCodeGenException("@ImmutableDefaults method must be private static void and have one argument of type 'Builder'", file, index + 1);
                }
            }
        }
        return null;
        ...
        for (int index = defLine; index < content.size(); index++) {
            if (content.get(index).trim().equals("@ImmutablePreBuild")) {
                ...
                if (index + 1 < content.size()) {
                    ...
                    throw new BeanCodeGenException("@ImmutablePreBuild method must be private static void and have one argument of type 'Builder'", file, index + 1);
                }
            }
        }
        return null;
