 ...
 */
package io.devpl.sdk.beans.serialize.xml;
...
import io.devpl.sdk.beans.serialize.JodaBeanSerializer;
import io.devpl.sdk.beans.serialize.SerCategory;
import io.devpl.sdk.beans.serialize.SerIterator;
import io.devpl.sdk.beans.serialize.SerOptional;
import io.devpl.sdk.beans.serialize.SerTypeMapper;
...
public class JodaBeanXmlWriter {
     ...
     * The settings to use.
     */
    private final JodaBeanSerializer settings;
    /**
     ...
     */
    public JodaBeanXmlWriter(JodaBeanSerializer settings) {
     ...
     */
    public JodaBeanXmlWriter(JodaBeanSerializer settings, StringBuilder builder) {
    ...
    public void write(Bean bean, boolean rootType, Appendable output) throws IOException {
        ...
        writeHeader();
        output.append('<').append(JodaBeanXml.BEAN);
        if (rootType) {
            appendAttribute(output, JodaBeanXml.TYPE, type);
        }
        output.append('>').append(settings.getNewLine());
        writeBean(rootBean, settings.getIndent());
        output.append('<').append('/').append(JodaBeanXml.BEAN).append('>').append(settings.getNewLine());
    }
    ...
    private void writeBean(String currentIndent, String tagName, StringBuilder attrs, Class<?> propType, Bean value) throws IOException {
        ...
        if (value.getClass() != propType) {
            ...
            appendAttribute(output, JodaBeanXml.TYPE, typeStr);
        }
    ...
    private void writeElements(String currentIndent, String tagName, StringBuilder attrs, SerIterator itemIterator) throws IOException {
        if (itemIterator.metaTypeRequired()) {
            appendAttribute(attrs, JodaBeanXml.METATYPE, itemIterator.metaTypeName());
        }
        if (itemIterator.category() == SerCategory.GRID) {
            appendAttribute(attrs, JodaBeanXml.ROWS, Integer.toString(itemIterator.dimensionSize(0)));
            appendAttribute(attrs, JodaBeanXml.COLS, Integer.toString(itemIterator.dimensionSize(1)));
        }
    ...
    private void writeElements(String currentIndent, SerIterator itemIterator) throws IOException {
        ...
        while (itemIterator.hasNext()) {
            ...
            if (keyConverter != null) {
                ...
                appendAttribute(attr, JodaBeanXml.KEY, keyStr);
            }
            if (rowConverter != null) {
                String rowStr = convertToString(rowConverter, itemIterator.key(), "table row");
                appendAttribute(attr, JodaBeanXml.ROW, rowStr);
                String colStr = convertToString(columnConverter, itemIterator.column(), "table column");
                appendAttribute(attr, JodaBeanXml.COL, colStr);
            }
            if (itemIterator.count() != 1) {
                appendAttribute(attr, JodaBeanXml.COUNT, Integer.toString(itemIterator.count()));
            }
            if (keyBean) {
                Object key = itemIterator.key();
                output.append(currentIndent).append('<').append(JodaBeanXml.ENTRY).append(attr).append('>').append(settings.getNewLine());
                writeKeyElement(currentIndent + settings.getIndent(), key, itemIterator);
                writeValueElement(currentIndent + settings.getIndent(), JodaBeanXml.ITEM, new StringBuilder(), itemIterator);
                output.append(currentIndent).append('<').append('/').append(JodaBeanXml.ENTRY).append('>').append(settings.getNewLine());
            } else {
                String tagName = itemIterator.category() == SerCategory.MAP ? JodaBeanXml.ENTRY : JodaBeanXml.ITEM;
    ...
    private void writeKeyElement(String currentIndent, Object key, SerIterator itemIterator) throws IOException {
        ...
        if (settings.getConverter().isConvertible(itemIterator.keyType())) {
            writeSimple(currentIndent, JodaBeanXml.ITEM, new StringBuilder(), Object.class, key);
        } else if (key instanceof Bean) {
            writeBean(currentIndent, JodaBeanXml.ITEM, new StringBuilder(), itemIterator.keyType(), (Bean) key);
        } else {
    ...
    private void writeValueElement(String currentIndent, String tagName, StringBuilder attrs, SerIterator itemIterator) throws IOException {
        ...
        if (value == null) {
            appendAttribute(attrs, JodaBeanXml.NULL, "true");
            output.append(currentIndent).append('<').append(tagName).append(attrs).append("/>").append(settings.getNewLine());
    ...
    private void writeSimple(String currentIndent, String tagName, StringBuilder attrs, Class<?> declaredType, Object value) throws IOException {
        ...
        if (declaredType == Object.class) {
            ...
            if (realType != String.class) {
                ...
                appendAttribute(attrs, JodaBeanXml.TYPE, typeStr);
            } else {
