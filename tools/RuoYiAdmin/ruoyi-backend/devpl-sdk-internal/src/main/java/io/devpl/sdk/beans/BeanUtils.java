...
public final class BeanUtils {
    ...
    public static boolean propertiesEqual(Bean bean1, Bean bean2) {
        ...
        for (String name : names) {
            Object value1 = bean1.getProperty(name).get();
            Object value2 = bean2.getProperty(name).get();
    ...
    public static int propertiesHashCode(Bean bean) {
        ...
        for (String name : names) {
            Object value = bean.getProperty(name).get();
            hash += hashCode(value);
        }
        return hash;
    }
    ...
    public static <T extends Bean> BeanBuilder<T> copy(Bean sourceBean, Class<T> destType) {
        ...
        @SuppressWarnings("unchecked") BeanBuilder<T> destBuilder = (BeanBuilder<T>) destMeta.builder();
    ...
    public static boolean equalIgnoring(Bean bean1, Bean bean2, MetaProperty<?>... properties) {
        BeanUtils.notNull(bean1, "bean1");
        BeanUtils.notNull(bean2, "bean2");
        BeanUtils.notNull(properties, "properties");
        if (bean1 == bean2) {
            return true;
        }
        ...
        switch (properties.length) {
            ...
            case 1: {
                ...
                for (MetaProperty<?> mp : bean1.metaBean().metaPropertyIterable()) {
                    if (!ignored.equals(mp) && !BeanUtils.equal(mp.get(bean1), mp.get(bean2))) {
                        return false;
                    }
                }
                return true;
            ...
            default:
                ...
                for (MetaProperty<?> mp : bean1.metaBean().metaPropertyIterable()) {
                    if (!ignored.contains(mp) && !BeanUtils.equal(mp.get(bean1), mp.get(bean2))) {
                        return false;
                    }
                }
                return true;
    ...
    private static final class Comp implements Comparator<Bean> {
        ...
        public int compare(Bean bean1, Bean bean2) {
            @SuppressWarnings("unchecked") Comparable<Object> value1 = (Comparable<Object>) query.apply(bean1);
