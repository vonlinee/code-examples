...
public final class JodaBeanTests {
    ...
    private static void coverProperties(Bean bean) {
        ...
        for (MetaProperty<?> mp : metaBean.metaPropertyIterable()) {
            ...
            ignoreThrows(() -> {
                Method m = metaBean.getClass()
                                   .getDeclaredMethod("propertySet", Bean.class, String.class, Object.class, Boolean.TYPE);
                m.setAccessible(true);
        ...
        ignoreThrows(() -> metaBean.builder()
                                   .set(JodaBeanTests.TEST_COVERAGE_PROPERTY, JodaBeanTests.TEST_COVERAGE_STRING));
        ignoreThrows(() -> bean.getProperty(JodaBeanTests.TEST_COVERAGE_PROPERTY));
    }
    ...
    private static void coverNonProperties(Bean bean) {
        ...
        if (metaBean instanceof DirectMetaBean) {
            DirectBeanMetaProperty<String> dummy = DirectBeanMetaProperty.ofReadWrite(metaBean, "foo_bar", metaBean.beanType(), String.class);
        ...
        for (String propertyName : propertyNameSet) {
            assertNotNull(bean.getProperty(propertyName));
        }
        assertThrows(() -> bean.getProperty(""), NoSuchElementException.class);
    ...
    private static List<Object> buildSampleConstants(Class<?> queryType, Class<?> targetType) {
        ...
        for (Field field : queryType.getFields()) {
            if (field.getType() == targetType && Modifier.isPublic(field.getModifiers()) && Modifier.isStatic(field.getModifiers()) && Modifier.isFinal(field.getModifiers()) && !field.isSynthetic()) {
    ...
    static {
        ...
        map.put(Collection.class, Collections.singletonList(new ArrayList<>()));
        map.put(List.class, Collections.singletonList(new ArrayList<>()));
        map.put(Set.class, Collections.singletonList(new HashSet<>()));
        map.put(SortedSet.class, Collections.singletonList(new TreeSet<>()));
        try {
            Class<?> cls = Class.forName("com.google.common.collect.ImmutableList");
            Method method = cls.getDeclaredMethod("of");
            map.put(cls, Collections.singletonList(method.invoke(null)));
        } catch (Exception ex) {
            // ignore
        }
        try {
    ...
    private static void assertThrows(AssertRunnable runner, Class<? extends Throwable> expected) {
        ...
        try {
            ...
            if (!expected.isInstance(ex)) {
                throw new AssertionError("Expected " + expected.getSimpleName() + " but received " + ex.getClass()
                                                                                                       .getSimpleName(), ex);
            }
        }
    }
