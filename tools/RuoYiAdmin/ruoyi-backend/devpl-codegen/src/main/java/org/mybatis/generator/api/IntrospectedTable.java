...
public abstract class IntrospectedTable {
     ...
     */
    protected final Map<InternalAttribute, String> internalAttributes = new EnumMap<>(InternalAttribute.class);
    ...
    private boolean isSubPackagesEnabled(PropertyHolder propertyHolder) {
        return StringUtils.isTrue(propertyHolder.getProperty(ConfigKeyRegistry.ANY_ENABLE_SUB_PACKAGES));
    }
    ...
    protected String calculateDynamicSqlSupportPackage() {
        ...
        String packageName = config.getProperty(ConfigKeyRegistry.CLIENT_DYNAMIC_SQL_SUPPORT_PACKAGE);
        if (StringUtils.hasLength(packageName)) {
            return packageName + fullyQualifiedTable.getSubPackageForClientOrSqlMap(isSubPackagesEnabled(config));
        } else {
    ...
    protected void calculateJavaClientAttributes() {
        ...
        sb.append('.');
        if (StringUtils.hasLength(tableConfiguration.getMapperName())) {
            sb.append(tableConfiguration.getMapperName());
        } else {
            if (StringUtils.hasLength(fullyQualifiedTable.getDomainObjectSubPackage())) {
        ...
        sb.append(calculateJavaClientInterfacePackage());
        sb.append('.');
        if (StringUtils.hasLength(tableConfiguration.getSqlProviderName())) {
        ...
        sb.append('.');
        if (StringUtils.hasLength(tableConfiguration.getDynamicSqlSupportClassName())) {
        ...
        if (StringUtils.hasLength(tableConfiguration.getDynamicSqlTableObjectName())) {
    ...
    protected String calculateJavaModelExamplePackage() {
        JavaModelGeneratorConfiguration config = context.getJavaModelGeneratorConfiguration();
        String exampleTargetPackage = config.getProperty(ConfigKeyRegistry.MODEL_GENERATOR_EXAMPLE_PACKAGE);
        if (!hasLength(exampleTargetPackage)) {
    ...
    protected String calculateSqlMapPackage() {
        ...
        if (config != null) {
            ...
            if (hasLength(tableConfiguration.getMapperName())) {
                String mapperName = tableConfiguration.getMapperName();
                int ind = mapperName.lastIndexOf('.');
                if (ind != -1) {
                    sb.append('.').append(mapperName, 0, ind);
                }
            } else if (hasLength(fullyQualifiedTable.getDomainObjectSubPackage())) {
    ...
    public boolean isImmutable() {
        ...
        if (tableConfiguration.getProperties().containsKey(ConfigKeyRegistry.ANY_IMMUTABLE)) {
            properties = tableConfiguration.getProperties();
        } else {
            properties = context.getJavaModelGeneratorConfiguration().getProperties();
        }

        return isTrue(properties.getProperty(ConfigKeyRegistry.ANY_IMMUTABLE));
    }
    ...
    public boolean isConstructorBased() {
        ...
        if (tableConfiguration.getProperties().containsKey(ConfigKeyRegistry.ANY_CONSTRUCTOR_BASED)) {
            properties = tableConfiguration.getProperties();
        } else {
            properties = context.getJavaModelGeneratorConfiguration().getProperties();
        }

        return isTrue(properties.getProperty(ConfigKeyRegistry.ANY_CONSTRUCTOR_BASED));
    }
