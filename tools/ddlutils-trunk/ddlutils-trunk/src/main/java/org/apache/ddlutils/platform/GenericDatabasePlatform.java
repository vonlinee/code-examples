...
public abstract class GenericDatabasePlatform extends JdbcSupport implements DatabasePlatform {
    ...
    protected Table findChangedTable(Database currentModel, TableChange change) throws ModelException {
        Table table = currentModel.findTable(change.getChangedTable(), getPlatformInfo().isDelimitedIdentifiersSupported());
    ...
    public void insert(Connection connection, Database model, DynaBean dynaBean) throws DatabaseOperationException {
        ...
        try {
            ...
            if (count != 1) {
                _log.warn("Attempted to insert a single row " + dynaBean + " in table " + dynaClass.getTableName() + " but changed " + count + " row(s)");
            }
        } catch (SQLException ex) {
    ...
    public void update(Connection connection, Database model, DynaBean dynaBean) throws DatabaseOperationException {
        ...
        try {
            ...
            if (count != 1) {
                _log.warn("Attempted to insert a single row " + dynaBean + " into table " + dynaClass.getTableName() + " but changed " + count + " row(s)");
            }
        } catch (SQLException ex) {
    ...
    public void update(Connection connection, Database model, DynaBean oldDynaBean, DynaBean newDynaBean) throws DatabaseOperationException {
        ...
        try {
            ...
            if (count != 1) {
                _log.warn("Attempted to insert a single row " + newDynaBean + " into table " + dynaClass.getTableName() + " but changed " + count + " row(s)");
            }
        } catch (SQLException ex) {
            throw new DatabaseOperationException("Error while updating in the database", ex);
        } finally {
    ...
    public void delete(Connection connection, Database model, DynaBean dynaBean) throws DatabaseOperationException {
        ...
        try {
            ...
            if (count != 1) {
                _log.warn("Attempted to delete a single row " + dynaBean + " in table " + dynaClass.getTableName() + " but changed " + count + " row(s).");
            }
        } catch (SQLException ex) {
