...
/**
 * 事件对象. 每个Event关联一个event source,event target和一个event type.
 * 1.event source: 指定了event handler被注册到哪个对象上，event source将发送事件给event handler
 * 2.event target: 定义了时间被提交后将经过的路径
 * 3.event type: 对Event对象进行额外的分类.
