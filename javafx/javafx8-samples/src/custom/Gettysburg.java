...
public class Gettysburg extends Application {
    ...
    public void start(Stage stage) {
        ...
        stackedContent.layoutBoundsProperty().addListener(new ChangeListener<Bounds>() {
            ...
            public void changed(ObservableValue<? extends Bounds> observableValue, Bounds oldValue, Bounds newValue) {
                paperView.setViewport(new Rectangle2D(newValue.getMinX(), newValue.getMinY(), Math.min(newValue.getWidth(), paper.getWidth()), Math.min(newValue.getHeight(), paper.getHeight())));
            }
        });
    ...
    private void makeScrollFadeable(final Node scroll) {
        ...
        scroll.hoverProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observableValue, Boolean wasHover, Boolean isHover) {
                if (!isHover) {
                    fader.playFromStart();
                } else {
                    fader.stop();
     ...
     */
    static class BorderedTitledPane extends StackPane {
