...
public class CodeMirrorEditor implements CodeEditor {
    ...
    public void init(Runnable... runAfterLoading) {
        try {
            File file = new File("D:\\Develop\\Code\\devpl\\fxui\\src\\main\\resources\\codemirror\\index.html");

            URL resource = Thread.currentThread().getContextClassLoader().getResource("/codemirror/index.html");
            resource = file.toURI().toURL();
            if (resource == null) {
                throw new RuntimeException("Editor初始化失败 加载codemirror失败");
            }
    ...
    public void setMode(String mode) {
        Platform.runLater(() -> webView.getEngine().executeScript("setMode(\"" + mode + "\");"));
    }
...
//    }
    ...
    public void setTheme(Theme theme, String... cssFile) {
        String argument = "'" + theme.getThemName() + "'";
        if (cssFile != null && cssFile.length > 0) {
            StringBuilder cssFileArgument = new StringBuilder();
            for (String file : cssFile) {
                cssFileArgument.append(", '").append(file).append("'");
