 ...
 * Events are posted ({@link #publish(Object)}) to the bus, which delivers it to subscribers that have a matching handler
...
public class DefaultEventBus implements EventBus {
    ...
    private void subscribe(Object subscriber, SubscriberMethod subscriberMethod) {
        ...
        if (subscriberMethod.sticky) {
            if (eventInheritance) {
                ...
                checkPostStickyEventToSubscription(newSubscription, stickyEvents.get(eventType));
            }
        }
    }
    ...
    private void postSingleEvent(Object event, PostingThreadState postingState) throws Error {
        ...
        if (eventInheritance) {
            ...
            int countTypes = eventTypes.size();
            for (int h = 0; h < countTypes; h++) {
                Class<?> eventTypeClass = eventTypes.get(h);
        ...
        if (!subscriptionFound) {
            ...
            if (sendNoSubscriberEvent && eventClass != NoSubscriberEvent.class &&
                ...
                // 此时发布状态isPosting为true
                publish(new NoSubscriberEvent(this, event));
            }
        }
    }
