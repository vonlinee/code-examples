...
public class ThreadSafeEventService implements EventService {
	...
	// Making these generic collections is a bad idea, it doesn't compile since it's
	// better to have all the maps
	 ...
	 *                                                    subscriber should spend
	 *                                                    handling an event, The
	 *                                                    service will publish an
	 *                                                    SubscriberTimingEvent
	 *                                                    after listener processing
	 *                                                    if the time was exceeded.
	 *                                                    If null, no
	 *                                                    EventSubscriberTimingEvent
	 *                                                    will be issued.
	 */
	 ...
	 *                                                    SubscriberTimingEvent
	 *                                                    internally and call the
	 *                                                    protected
	 *                                                    subscribeTiming() method
	 *                                                    when they occur. This logs
	 *                                                    a warning to the
	 *                                                    {@link Logger} by default.
	 */
	public ThreadSafeEventService(Long timeThresholdForEventTimingEventPublication,
	 ...
	 *                                  is null and subscribeTimingEventsInternally
	 *                                  is true.
	 */
	public ThreadSafeEventService(Long timeThresholdForEventTimingEventPublication,
			boolean subscribeTimingEventsInternally, Integer cleanupStartThreshold, Integer cleanupStopThreshold,
	 ...
	 * 
	 * @param cleanupPeriodMS interval in milliseconds between cleanup runs. Passing
	 *                        null stops cleanup.
	 */
	...
	/**
	 * All veto subscriptions methods call this method. Extending classes only have
	 * to override this method to subscribe all veto subscriptions.
	 *
	 * @param subscription    the topic, Pattern, or event class to subscribe to
	 * @param vetoListenerMap the internal map of veto listeners to use (by topic of
	 *                        class)
	 ...
	 * <p/>
	 * Overriding this method is only for the adventurous. This basically gives you
	 ...
	 * @param subscriberMap              the internal map of subscribers to use (by
	 *                                   topic or class)
	 * @param subscriber                 the EventSubscriber or EventTopicSubscriber
	 *                                   to subscribe, or a WeakReference to either
	 *
		...
		synchronized (listenerLock) {
			...
			if (currentSubscribers == null) {
				...
				for (Iterator iterator = currentSubscribers.iterator(); iterator.hasNext();) {
					...
					Object realCurrentSubscriber = getRealSubscriberAndCleanStaleSubscriberIfNecessary(iterator,
							currentSubscriber);
	...
	/**
	 * All event subscriber unsubscriptions call this method. Extending classes only
	 * have to override this method to subscribe all subscriber unsubscriptions.
	 *
	 ...
	 * @param vl              the veto listener to unsubscribe, or a WeakReference
	 *                        to one
	 *
	...
	/**
	 * All publish methods call this method. Extending classes only have to override
	 * this method to handle all publishing cases.
	 *
	 * @param event           the event to publish, null if publishing on a topic
	 * @param topic           if publishing on a topic, the topic to publish on,
	 *                        else null
	 * @param eventObj        if publishing on a topic, the eventObj to publish,
	 *                        else null
	...
	protected void publish(final Object event, final String topic, final Object eventObj, final List subscribers,
			final List vetoSubscribers, StackTraceElement[] callingStack) {
	...
	private List sortSubscribers(List subscribers) {
		...
		if (prioritizedSubscribers == null) {
			...
			// Issue 26 - of all priorities are negative, then add the FIFO after processing
			// all of them
			...
			for (Iterator vlIter = vetoSubscribers.iterator(); vlIter.hasNext();) {
				...
				try {
					...
					if (shouldVeto) {
						...
						if (LOG.isLoggable(Level.DEBUG)) {
							LOG.debug("Publication vetoed. Event:" + event + ", Topic:" + topic + ", veto subscriber:"
									+ vl);
						}
						return true;
					}
	...
	/**
	 * Adds an event to the event cache, if appropriate. This method is called just
	 * before publication to listeners, after the event passes any veto listeners.
	 * <p/>
	...
	protected void addEventToCache(Object event, String topic, Object eventObj) {
		// Taking the listener lock here, since a listener that is now subscribing will
		// want
...
//         }
		/*
		 * } else if (type instanceof TypeVariable<?>) { TypeVariable<?> v =
		 * (TypeVariable<?>)type; out.print(v.getName()); } else if (type instanceof
		 * GenericArrayType) { GenericArrayType a = (GenericArrayType)type;
		 * printType(a.getGenericComponentType()); out.print("[]"); } else if (type
		 * instanceof WildcardType) { WildcardType w = (WildcardType)type; Type[] upper
		 * = w.getUpperBounds(); Type[] lower = w.getLowerBounds(); if (upper.length==1
		 * && lower.length==0) { out.print("? extends "); printType(upper[0]); } else if
		 * (upper.length==0 && lower.length==1) { out.print("? super ");
		 * printType(lower[0]); } else assert false; }
		 */
	}
	 ...
	 */
	protected void handleVeto(VetoEventListener vl, Object event, VetoTopicEventListener vtl, String topic,
			Object eventObj) {
		if (LOG.isLoggable(Level.DEBUG)) {
			if (event != null) {
	...
	/**
	 * Given a Map (of Lists of subscribers or veto listeners), removes the toRemove
	 * element from the List in the map for the given key. The entire map is checked
	 * for WeakReferences and ProxySubscribers and they are all unsubscribed if
	 * stale.
	 *
	...
	/**
	 * Set the number of events cached for a particular class of event. By default,
	 * no events are cached.
	 * <p/>
	 ...
	 * A, X and Y, and X and Y are both derived from A, then setting the cache size
	 * for A applies the cache size for all three. Setting the cache size for X
	 * applies to X and leaves the settings for A and Y in tact. Interfaces can be
	 * passed to this method, but they only take effect if the cache size of a class
	 * or it's superclasses has been set. Just like Class.getInterfaces(), if
	 * multiple cache sizes are set, the interface names declared earliest in the
	 * implements clause of the eventClass takes effect.
	 * <p/>
	 * The cache for an event is not adjusted until the next event of that class is
	 * published.
	 *
	...
	/**
	 * Returns the number of cached data objects published on a particular topic. By
	...
	protected void subscribeVetoException(final Object event, final String topic, final Object eventObj, Throwable e,
			StackTraceElement[] callingStack, VetoEventListener vetoer) {
	...
	/** Called during event handling exceptions, calls handleException */
	protected void handleException(final Object event, Throwable e, StackTraceElement[] callingStack,
	 ...
	 */
	protected void handleException(final String action, final Object event, final String topic, final Object eventObj,
		...
		String contextMsg = "Exception " + action + " event class=" + eventClassString + ", event=" + eventString
				+ ", topic=" + topic + ", eventObj=" + eventObj;
	 ...
	 * <p/>
	 * Not private since I don't claim I'm smart enough to anticipate all needs, but
	 * I am smart enough to doc the rules you must follow to override this method.
	 * Those rules may change (changes will be doc'ed), override at your own risk.
	 * <p/>
	 * Overriders MUST call iterator.remove() to unsubscribe the proxy if the
	 * subscriber is a ProxySubscriber and is stale and should be cleaned up. If the
	 * ProxySubscriber is unsubscribed, then implementers MUST also call
	 * proxyUnsubscribed() on the subscriber. Overriders MUST also remove the proxy
	 * from the weakProxySubscriber list by calling removeStaleProxyFromList. Method
	 * assumes caller is holding the listenerList lock (else how can you pass the
	 * iterator?).
	 * 
	...
	class CleanupTimerTask extends TimerTask {
		...
		public void run() {
			synchronized (listenerLock) {
				ThreadSafeEventService.this
						.publish(new CleanupEvent(CleanupEvent.Status.STARTING, weakRefPlusProxySubscriberCount, null));
				if (weakRefPlusProxySubscriberCount <= cleanupStopThreshold) {
					...
					ThreadSafeEventService.this
							.publish(new CleanupEvent(CleanupEvent.Status.UNDER_STOP_THRESHOLD_CLEANING_CANCELLED,
									weakRefPlusProxySubscriberCount, null));
					return;
				}
				LOG.debug("Starting a weak reference and proxy cleanup.");
				ThreadSafeEventService.this.publish(new CleanupEvent(
						CleanupEvent.Status.OVER_STOP_THRESHOLD_CLEANING_BEGUN, weakRefPlusProxySubscriberCount, null));
				...
				for (Map subscriberMap : allSubscriberMaps) {
					...
					for (Object subscription : subscriptions) {
						...
						for (Iterator iter = subscribers.iterator(); iter.hasNext();) {
							Object subscriber = iter.next();
							Object realSubscriber = getRealSubscriberAndCleanStaleSubscriberIfNecessary(iter,
									subscriber);
							if (realSubscriber == null) {
								staleCount++;
							}
				...
				ThreadSafeEventService.this.publish(new CleanupEvent(CleanupEvent.Status.FINISHED_CLEANING,
						weakRefPlusProxySubscriberCount, staleCount));
			}
		}
	}
