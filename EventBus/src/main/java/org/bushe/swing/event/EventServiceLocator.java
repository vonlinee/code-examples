 ...
 * {@link #SERVICE_NAME_SWING_EVENT_SERVICE} and returned by
 * {@link #getSwingEventService()}. Also by default this same instance is
 * returned by {@link #getEventBusService()}, is mapped to
 * {@link #SERVICE_NAME_EVENT_BUS} and wrapped by the EventBus.
 * <p/>
 * Since the default EventService implementation is thread safe, and since it's
 * not good to have lots of events on the EventDispatchThread you may want
 * multiple EventServices running on multiple threads, perhaps pulling events
 * from a server and coalescing them into one or more events that are pushed
 * onto the EDT.
 * <p/>
 * To change the default implementation class for the EventBus' EventService,
 * use the API:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * Likewise, you can set this on the command line via
 * -Dorg.bushe.swing.event.swingEventServiceClass=foo.YourEventServiceImpl
 * <p/>
 * To change the default implementation class for the SwingEventService, use the
 * API:
 * 
 * <pre>
 * EventServiceLocator.setEventService(EventServiceLocator.SERVICE_NAME_SWING_EVENT_SERVICE,
 * 		new SomeSwingEventServiceImpl());
 * </pre>
 * 
 * Or use system properties by:
 * 
...
public class EventServiceLocator {
	/**
	 * The name "EventBus" is reserved for the service that the EventBus wraps and
	 * is returned by {@link #getEventBusService}.
	 */
	 ...
	 * {@link #getSwingEventService}.
	 */
	public static final String SERVICE_NAME_SWING_EVENT_SERVICE = "SwingEventService";

	/**
	 * Set this Java property to a Class that implements EventService to use an
	 * instance of that class instead of the instance returned by
	 * {@link #getSwingEventService}. Must be set before
	 * {@link #getEventBusService()} is called.
	 */
	public static final String EVENT_BUS_CLASS = "org.bushe.swing.event.eventBusClass";
	/**
	 * Set this Java property to a Class that implements EventService to use an
	 * instance of that class instead of {@link SwingEventService} as service
	 * returned by {@link #getSwingEventService}. Must be set on startup or before
	 * the method {@link #getSwingEventService}is called.
	 */
	...
	/**
	 * Registers a named EventService to the locator. Can be used to change the
	 ...
	 *                                     and the new service is non-null
	 */
	...
	/**
	 * Use this carefully. Clears all the event services, including the
	 * SwingEventService (used by EventBus).
	 * <p>
	...
	private static synchronized EventService getEventService(String eventServiceClassPropertyName,
			EventService defaultService) {
		EventService result;
		...
		if (eventServiceClassName != null) {
			...
			try {
				...
				throw new RuntimeException("Could not find class specified in the property "
			...
			try {
				...
				throw new RuntimeException("InstantiationException creating instance of class set from Java property"
						+ eventServiceClassPropertyName + ".  Class=" + eventServiceClassName, e);
			} catch (IllegalAccessException e) {
				throw new RuntimeException("IllegalAccessException creating instance of class set from Java property"
						+ eventServiceClassPropertyName + ".  Class=" + eventServiceClassName, e);
			}
			try {
				...
				throw new RuntimeException("ClassCastException casting to " + EventService.class
						+ " from instance of class set from Java property" + eventServiceClassPropertyName + ".  Class="
						+ eventServiceClassName, ex);
			}
		} else {
