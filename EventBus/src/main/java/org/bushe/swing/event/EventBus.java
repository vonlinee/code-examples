 ...
 * simple static wrapper around a global instance of an {@link EventService},
 * specifically a {@link SwingEventService} by default.
 * <p/>
 * For Swing Applications the EventBus is nearly all you need, besides some of
 * your own Event classes (if so desired).
 * <p/>
 * The EventBus is really just a convenience class that provides a static
 * wrapper around a global {@link EventService} instance. This class exists
 * solely for simplicity. Calling <code>EventBus.subscribeXXX/publishXXX</code>
 * is equivalent to
 * <code>EventServiceLocator.getEventBusService().subscribeXXX/publishXXX</code>,
 * it is just shorter to type. See
 * {@link org.bushe.swing.event.EventServiceLocator} for details on how to
 * customize the global EventService in place of the default SwingEventService.
 *
...
public class EventBus {
	...
	/**
	 * The EventBus uses a global static EventService. This method is not necessary
	 * in usual usage, use the other static methods instead. It is used to expose
	 * any other functionality and for framework classes (EventBusAction)
	 *
