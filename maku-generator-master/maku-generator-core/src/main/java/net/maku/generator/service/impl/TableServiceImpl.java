...
public class TableServiceImpl extends BaseServiceImpl<TableDao, TableEntity> implements TableService {
    ...
    public void tableImport(Long datasourceId, String tableName) {
        ...
        // 获取原生字段数据
        List<TableFieldInfo> tableFieldList = GenUtils.getTableFieldList(dataSource, table.getId(), table.getTableName());
        // 初始化字段数据
    ...
    public void sync(Long id) {
        ...
        // 从数据库获取表字段列表
        List<TableFieldInfo> dbTableFieldList = GenUtils.getTableFieldList(datasource, table.getId(), table.getTableName());
        if (dbTableFieldList.size() == 0) {
            throw new ServerException("同步失败，请检查数据库表：" + table.getTableName());
        }

        List<String> dbTableFieldNameList = dbTableFieldList.stream().map(TableFieldInfo::getFieldName).collect(Collectors.toList());

        // 表字段列表
        List<TableFieldInfo> tableFieldList = tableFieldService.getByTableId(id);

        Map<String, TableFieldInfo> tableFieldMap = tableFieldList.stream().collect(Collectors.toMap(TableFieldInfo::getFieldName, Function.identity()));
        ...
        dbTableFieldList.forEach(field -> {
            ...
            // 修改字段
            TableFieldInfo updateField = tableFieldMap.get(field.getFieldName());
        ...
        // 删除数据库表中没有的字段
        List<TableFieldInfo> delFieldList = tableFieldList.stream().filter(field -> !dbTableFieldNameList.contains(field.getFieldName())).collect(Collectors.toList());
        if (delFieldList.size() > 0) {
            List<Long> fieldIds = delFieldList.stream().map(TableFieldInfo::getId).collect(Collectors.toList());
