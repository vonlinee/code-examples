 ...
 * EventDispatchThread, subscribers will receive the event on the calling thread, and not the EventDispatchThread.  Swing components
 * should use the SwingEventService instead, which is the implementation used by the EventBus.
 * <p/>
 * 两个线程可能同时访问ThreadSafeEventService
 * 1.一个解除订阅监听器 A
 * 2.一个发布主题 A
 ...
 * TODO: 2022/11/11   (param) a JMS-like selector (can be done in base classes by implements like a commons filter
...
public class ThreadSafeEventService implements EventService {
    ...
    private static final Comparator<Prioritized> PRIORITIZED_SUBSCRIBER_COMPARATOR = new PrioritizedSubscriberComparator();
    ...
    private Map<Class, List> cacheByEvent = new HashMap<>();
    ...
    private Map<String, List> cacheByTopic = new HashMap<>();
    ...
    public ThreadSafeEventService(Long timeThresholdForEventTimingEventPublication,
        ...
        if (subscribeTimingEventsInternally) {
            ...
            subscribeStrongly(SubscriberTimingEvent.class, event -> subscribeTiming((SubscriberTimingEvent) event));
        }
    ...
    /**
     * @see EventService#subscribe(Type, EventSubscriber)
     */
    @Override
    ...
    /**
     * @see EventService#publish(Type, Object)
     */
    ...
    public <T> List<T> getSubscribers(String topic) {
        ...
        List patternMatches;
        synchronized (listenerLock) {
            exactMatches = getSubscribers(topic, subscribersByTopic);
    ...
    private List<Object> getSubscribers(Object classOrTopic, Map<Object, Object> subscriberMap) {
        List<Object> result;
        synchronized (listenerLock) {
            List<Object> subscribers = (List<Object>) subscriberMap.get(classOrTopic);
...
//         }
    ...
    private void checkTimeLimit(long start, Object event, EventSubscriber subscriber, VetoEventListener l) {
        ...
        if (end - start > timeThresholdForEventTimingEventPublication) {
            publish(new SubscriberTimingEvent(this, start, end, timeThresholdForEventTimingEventPublication, event, subscriber, l));
        }
    }
     ...
     * @see #setCacheSizeForTopic(Pattern, int)
     */
