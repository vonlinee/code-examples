...
public class MyBatisCodeGenerator {
    ...
    // 保存哪些表需要进行代码生成
    private final Map<String, TableCodeGeneration> tablesForGeneration = new HashMap<>();
    ...
    public void generate() throws Exception {
        ...
        for (Map.Entry<String, TableCodeGeneration> entry : tablesForGeneration.entrySet()) {
            TableCodeGeneration generation = entry.getValue();
            ConnectionConfig connectionInfo = generation.getConnectionInfo();
            // 单表的配置选项
            ...
            // 数据库schema
            String dbType = generation.getConnectionInfo().getDbType();
            if (JDBCDriver.MYSQL5.name().equals(dbType) || JDBCDriver.MYSQL8.name().equals(dbType)) {
                tableConfig.setSchema(generation.getDbName());
                // 由于beginningDelimiter和endingDelimiter的默认值为双引号(")，在Mysql中不能这么写，所以还要将这两个默认值改为`
                context.addProperty(StringKey.BEGINNING_DELIMITER, "`");
                context.addProperty(StringKey.ENDING_DELIMITER, "`");
            } else {
                tableConfig.setCatalog(generation.getConnectionInfo().getSchema());
            }
            // 针对 postgresql 单独配置
            if (JDBCDriver.POSTGRE_SQL.name().equals(dbType)) {
            ...
            if (option.isUseSchemaPrefix()) {
                if (JDBCDriver.MYSQL5.name().equals(dbType) || JDBCDriver.MYSQL8.name().equals(dbType)) {
                    tableConfig.setSchema(generation.getDbName());
                } else if (JDBCDriver.ORACLE.name().equals(dbType)) {
                    // Oracle的schema为用户名，如果连接用户拥有dba等高级权限，若不设schema，会导致把其他用户下同名的表也生成一遍导致mapper中代码重复
                    tableConfig.setSchema(connectionInfo.getUsername());
                } else {
                    tableConfig.setCatalog(connectionInfo.getSchema());
                }
            }
            ...
            if (StringUtils.isNotEmpty(option.getGenerateKeys())) {
                ...
                if (JDBCDriver.MYSQL5.name().equals(dbType2) || JDBCDriver.MYSQL8.name().equals(dbType)) {
                    dbType2 = "JDBC";
                ...
                tableConfig.setGeneratedKey(new GeneratedKey(option.getGenerateKeys(), dbType2, true, null));
            }

            prepareJdbcConnectionConfig(context, dbType, connectionInfo);
    ...
    public void prepareContextProperties(Context context) {
        // 父包名
     ...
     * @param generation
     */
    public void preparePlugins(Context context, TableCodeGeneration generation) {
        TableCodeGenOption option = generation.getOption();
        String dbType = generation.getConnectionInfo().getDbType();
    ...
    public void prepareJdbcConnectionConfig(Context context, String dbType, ConnectionConfig connectionConfig) {
        ...
        jdbcConfig.setDriverClass(driver.getDriverClassName());
        jdbcConfig.setConnectionURL(connectionConfig.getConnectionUrl());
        jdbcConfig.setUserId(connectionConfig.getUsername());
        jdbcConfig.setPassword(connectionConfig.getPassword());
        if (JDBCDriver.ORACLE.name().equals(dbType)) {
     ...
     */
    public void prepareCommonConfig(Context context, GenericConfiguration generatorConfig) {
        // java model
