 ...
 */
public class CodeEditorLineNumberFactory<PS> implements IntFunction<Node> {

    private static final Insets DEFAULT_INSETS = new Insets(0.0, 5.0, 0.0, 5.0);
    private static final Paint DEFAULT_TEXT_FILL = Color.web("#666");
    private static final Font DEFAULT_FONT = Font.font("consolas", FontPosture.REGULAR, 14);
    private static final Font DEFAULT_FOLD_FONT = Font.font("consolas", FontWeight.BOLD, 14);
    ...
    public static <PS> IntFunction<Node> get(GenericStyledArea<PS, ?, ?> area, IntFunction<String> format) {
        if (area instanceof StyleClassedTextArea) {
            ...
            return get(classArea, format, styleList -> styleList != null && styleList.contains("collapse"), styleList -> {
            ...
            return get(inlineArea, format, pstyle -> pstyle != null && pstyle.contains("collapse"), pstyle -> pstyle.replaceFirst("visibility: collapse;", ""));
        }
    ...
    public static <PS> IntFunction<Node> get(GenericStyledArea<PS, ?, ?> area, IntFunction<String> format, Predicate<PS> isFolded, UnaryOperator<PS> removeFoldStyle) {
        return new CodeEditorLineNumberFactory<>(area, format, isFolded, removeFoldStyle);
    }
    ...
    private CodeEditorLineNumberFactory(GenericStyledArea<PS, ?, ?> area, IntFunction<String> format, Predicate<PS> isFolded, UnaryOperator<PS> removeFoldStyle) {
        ...
        if (isFoldedCheck != null) {
            area.getParagraphs().sizeProperty().addListener((ob, ov, nv) -> {
                if (nv <= ov) Platform.runLater(this::deleteParagraphCheck);
                else Platform.runLater(this::insertParagraphCheck);
            });
        }
    }
    ...
    public Node apply(int idx) {
        ...
        if (isFoldedCheck != null) {
            ...
            if (area.getParagraphs().size() > idx + 1) {
                if (isFoldedCheck.test(area.getParagraph(idx + 1)
                        .getParagraphStyle()) && !isFoldedCheck.test(area.getParagraph(idx).getParagraphStyle())) {
