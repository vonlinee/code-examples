...
public class CodeEditor extends StyleClassedTextArea {
...

    {
        ...
        // 行号
        this.setParagraphGraphicFactory(CodeEditorLineNumberFactory.get(this));

        setStyle("-fx-font-family: Consolas; -fx-font-size: 15");
    }

    /**
     * Creates an area that can render and edit the same {@link EditableStyledDocument} as another {@link CodeArea}.
     */
    public CodeEditor(@NamedArg("document") EditableStyledDocument<Collection<String>, String, Collection<String>> document) {
     ...
     */
    public CodeEditor(@NamedArg("text") String text) {
        this();
        appendText(text);
