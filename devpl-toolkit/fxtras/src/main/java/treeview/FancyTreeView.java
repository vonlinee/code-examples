...
public class FancyTreeView<T extends TreeItemObject> extends TreeView {
...
//                    Platform.runLater(() -> _ops_handler.handleDoubleClick(cell, e.isControlDown(), e.isShiftDown(), e.isAltDown()));
    ...
    public void expandAll() {
        TreeItem<TreeItemObject> root = getRoot();
        expandNodeAndChilren(root);
    }

    private void expandNodeAndChilren(TreeItem<TreeItemObject> node) {
    ...
    private void collapseNodeAndChildren(TreeItem<TreeItemObject> node) {
    ...
    public int findIndexOfVisibleItem(TreeItem target_item) {
        int index = 0;
        TreeItem<TreeItemObject> item = getTreeItem(index);
        while (item != null) {
    ...
    public void setRoot(TreeItemObject root_facade) {
