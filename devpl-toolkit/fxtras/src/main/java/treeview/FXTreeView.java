...
public class FXTreeView<T extends TreeModel> extends TreeView {
    @SuppressWarnings("WeakerAccess") // part of public API
    public FXTreeView(FancyTreeOperationHandler ops_handler) {
    ...
    public FXTreeView(FancyTreeOperationHandler ops_handler, boolean enable_dnd) {
...
//                    Platform.runLater(() -> _ops_handler.handleDoubleClick(cell, e.isControlDown(), e.isShiftDown(), e.isAltDown()));
        ...
        getSelectionModel().selectedItemProperty()
                .addListener((observable, oldValue, newValue) -> _ops_handler.selectionChanged(getSelectionModel().getSelectedItems()));
        ...
        addEventFilter(MouseEvent.MOUSE_RELEASED, e -> {
            ...
            if (e.getButton() == MouseButton.SECONDARY) {
                ...
                if (_context_menu != null) _context_menu.show(this, e.getScreenX(), e.getScreenY());
            }
        });
    }
    ...
    public void expandAll() {
        TreeItem<TreeModel> root = getRoot();
        expandNodeAndChilren(root);
    }

    private void expandNodeAndChilren(TreeItem<TreeModel> node) {
    ...
    private void collapseNodeAndChildren(TreeItem<TreeModel> node) {
    ...
    private TreeItem<T> findItemForModelNode(TreeItem<T> item, Object node) {
        if (item.getValue().getModelNode() == node) return item;
    ...
    public int findIndexOfVisibleItem(TreeItem target_item) {
        int index = 0;
        TreeItem<TreeModel> item = getTreeItem(index);
        while (item != null) {
            if (item == target_item) return index;
            index++;
    ...
    public void setRoot(TreeModel root_facade) {
